...
correct Ruby interpreter or libraries installed. It is strongly recommended that you try to run from the sources if you
can manage it.
...
Install Ruby and the FOX gui toolkit (instructions specifically for Ubuntu/Debian):
...
After installing Ruby, you must install a few required libraries. This will take a minute or two to automatically
download and install the libraries. Open a command window and type:
...
===== Linux
  ...
  sudo gem1.9.1 install fxruby --no-ri --no-rdoc
  sudo gem1.9.1 install i18n --no-ri --no-rdoc
...
Just clicking on the application probably won't work unless you've manually set up some aliases and paths. However, this should
work regardless:
