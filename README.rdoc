...
a Ruby interpreter or libraries installed.
...
Install Ruby 1.8.7 and the FOX gui toolkit (specifically for Ubuntu/Debian), by running 'flipped/install_ruby.sh').

===== Mac

Install Ruby 1.8.7 using the OS X 10.5 (Leopard) installer (http://rubyosx.rubyforge.org/). If you are using OS X 10.3
(Tiger) or 10.4 (Panther) you _should_ be fine using the Ruby 1.8.6 installers.
...
After installing Ruby, you must install a few required libraries (specifically FXRuby (http://www.fxruby.org/) and
